...
export class AdminDashboardComponent implements OnInit {
  ...
  loadLastBackupTime(): void {
    this.http.get<{ lastBackupTime: string }>(`${environment.apiUrl}/api/admin/database/backup/last-time`)
      .subscribe({
        next: (data) => {
        ...
        error: (err) => {
          console.error('Error loading last backup time:', err);
          this.lastBackupTime = 'Không thể lấy dữ liệu';
        }
      });
  }
